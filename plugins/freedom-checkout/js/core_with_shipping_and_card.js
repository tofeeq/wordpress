...
/*
 	...
 	*/
 	...
 	jQuery("#first-name, #last-name, #username, #email, #phone, #zip, #street-1, #street-2, #city, #zip-ship, #street-1-ship, #street-2-ship, #city-ship").on('keyup blur change click input', function(){
 ...
 */
 function requiredAction(input,valid,msg){
 	jQuery(input).closest('.control-group .controls').find('.help-block').html(msg);
 	if(!valid){
 		jQuery(input).closest('.control-group').removeClass('success').addClass('error');
 		return false;
 	}
 	else{
 		jQuery(input).closest('.control-group').removeClass('error').addClass('success');
 		return true;
 	}
 }
...
function validateRefererName(){
	...
	if(input.length > 4){ var retval = true; }
